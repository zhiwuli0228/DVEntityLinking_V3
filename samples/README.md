...
V2 新增 KPI/网元类实体的 `aliases` 默认必须为空数组；只有用户显式确认的别名才能写入。

V3 两层存储与 NER 样例：

- `real/v3_gauss_entities.json`：高斯 Mock 结构化实体样例，schema 为 `v3.gauss_entities.1`，当前 7 个实体，只保留 `entity_id`、`entity_type`、`canonical_name`、`aliases`、`description`。
- `real/v3_redis_entity_words.json`：Redis Mock 实体词 KV 样例，schema 为 `v3.redis_entity_words.1`，key 来源限于 `canonical_name` 和经确认 `aliases`，value 为单实体 ID。
