...
当前项目已关闭 V0 和 V1；V2 验收候选已撤回，前端视觉风格补救已完成 after 视觉接受，但 before 同场景截图或替代 before 证据口径仍未确认。用户已确认搁置 V2 遗留问题并进入 V3。V3 的两层存储、Redis/Gauss Mock 和 NER 详细设计保留为历史基线。V4 功能与代码基线已经完整，V4.1 负责将默认运行链与历史资产解耦，并提供可验证迁移治理；真实生产切换仍须在获授权环境执行。

## 当前状态

- 当前版本状态：V4 功能与代码基线完整；未声明真实生产已切换。
- 当前活跃迭代：V4.1 解耦与迁移治理；默认运行时禁止使用历史资产，外部数据发布、IR 冒烟和灰度待获授权环境执行。
- 当前权威状态与剩余发布事项见 `docs/PROJECT.md`、`docs/releases/V4.md` 和 `docs/baselines/v4.1/PRE_PRODUCTION_CHECKLIST.md`。
...
| `docs/baselines/v4/` | V4 IR/SR 主输出件。 |
| `docs/baselines/v4.1/` | V4.1 IR/SR、迁移追踪、兼容性差异、弃用登记和上线前检查。 |
...
| `docs/releases/V4.md` | V4 功能与代码基线完整性记录；未声明真实生产已切换。 |
